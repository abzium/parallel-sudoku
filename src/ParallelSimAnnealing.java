...
public class ParallelSimAnnealing {

    int[][] sudokuBoard1;
    int[][] sudokuBoard2;

    public ParallelSimAnnealing(int[][] sudoku) {
        sudokuBoard1 = new int[sudoku.length][];
        for (int i = 0; i < sudoku.length; i++)
            sudokuBoard1[i] = sudoku[i].clone();

        sudokuBoard2 = new int[sudoku.length][];
        for (int i = 0; i < sudoku.length; i++)
            sudokuBoard2[i] = sudoku[i].clone();

    }

    public void solve() {
        AtomicBoolean solutionFound = new AtomicBoolean();
        Thread thread1 = new Thread(new SimAnnealingThread(sudokuBoard1, solutionFound, 1));
        Thread thread2 = new Thread(new SimAnnealingThread(sudokuBoard2, solutionFound, 2));
    ...
    public class SimAnnealingThread implements Runnable {
        int[][] sudoku;
        int size;
        AtomicBoolean solutionFoundBoolean;
        int threadNumber;
        boolean threadFoundSol;

        public SimAnnealingThread(int[][] sudoku, AtomicBoolean solutionFound, int threadNumber) {
            this.sudoku = sudoku;
            this.size = sudoku.length;
            this.solutionFoundBoolean = solutionFound;
            this.threadNumber = threadNumber;
            threadFoundSol = false;
        }

        public void run() {
            while (!solutionFoundBoolean.get()) {
                ...
                if (score <= 0) {
                    solutionFoundBoolean.set(true);
                    System.out.println("Thread " + threadNumber + " found the solution!");
                    threadFoundSol = true;
                }
        ...
        private ArrayList<ArrayList<ArrayList<Integer>>> chooseNewState(int[][] currentSudoku, int[][] fixedSudoku,
            ...
            ArrayList<ArrayList<ArrayList<Integer>>> proposal = proposedState(convert2dArray(currentSudoku),
                    fixedSudoku,
                    listOfBlocks);
            // Extract data
