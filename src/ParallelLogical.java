...
public class ParallelLogical {
  ...
  public static interface FallbackSolver {
     ...
     * @param states The list of states to be added to, which will be iterated through.
     * @return Whether to continue iterating through the fallbacks. {@code false} if the board was solved already.
     */
    boolean recurse(int[][] sudoku, boolean[][][] candidates, List<BoardState> states);

    /**
     * Finds a cell with the fewest candidates, and tries each of them.
     * <p>Breadth-first search.
     */
    FallbackSolver guessAndCheck = (sudoku, candidates, states) -> {
      ...
      if (minCandidates > SIZE) return false;
     ...
     */
    FallbackSolver die = (sudoku, candidates, states) -> {
      for (int i = 0; i < SIZE; i++)
        for (int j = 0; j < SIZE; j++)
          if (sudoku[i][j] == UNKNOWN) return true;
      return false;
    };
  }
  ...
  private final List<BoardState> states = new ArrayList<>();
  ...
  public void solve() {
    try {
      ...
      final ForkJoinPool pool = new ForkJoinPool(NUM_TASKS);
      try {
				final ForkJoinTask<BoardState>[] tasks = new ForkJoinTask[NUM_TASKS];
        for (int i = 0; i < tasks.length; i++)
          tasks[i] = pool.submit(new SolverTask(state));
        join: for (int i = tasks.length; i-- > 1;) {
          try {
            state = tasks[i].join();
            return;
   ...
   * if there are, this must be called before {@link #trySolve(FallbackSolver)}, otherwise behavior is undefined.
  ...
  private static class SolverTask extends RecursiveTask<BoardState> {
    private BoardState state;
     ...
     * <p>Assumes the {@link BoardState#candidates} have already been initialized; usually this means having called {@link ParallelLogical#init}.
    ...
    public void trySolve() throws UnsolvableException {
      while (true) {
        ...
        // TODO: wait on other threads to complete?
        if (!FallbackSolver.die.recurse(state.sudoku, state.candidates, null)) return;
        else throw new UnsolvableException("augh");
      }
    }
